import {Alert, Keyboard, StatusBar, StyleSheet, Text, TextInput, TouchableOpacity, View} from 'react-native';
...
export default function ChatRoom() {
    ...
    const handleSendMessages = async () => {
        let messages = textRef.current.trim()
        if(!messages) return
        try {
            let roomId = getRoomId(user?.userId, item?.userId)
            const docRef = doc(db, 'rooms', roomId)
    ...
    return (
        <CustomKeyboardView inChat={true}>
            <View className={"flex-1 bg-white"}>
                ...
                <View className="h-3 border-b border-neutral-100"/>
                    ...
                    <View style={{marginBottom: hp(1.7)}} className="pt-2">
                        ...
                        <View
                            ...
                            <TouchableOpacity onPress={() => handleSendMessages()}
                                className='flex-row items-center bg-neutral-200 p-2 mr-[1px] rounded-full'>
